...
router.post('/seed-all', async (req, res) => {
  try {
    ...
    for (const p of products) {
      ...
      await db('products').insert({ slug: p.slug, title: p.title, short_description: p.short_description, price_eur: p.price_eur, price_usd: p.price_usd, page_count: p.page_count, dpi: p.dpi, age_group: p.age_group, theme: p.theme, badge: p.badge, is_featured: p.is_featured, is_published: p.is_published, preview_images: '[]', pdf_filename: p.pdf_filename, bonus_filename: null, description: null });
      inserted++;
    }
...
router.post('/seed-books', async (req, res) => {
  try {
    ...
    for (const p of books) {
      ...
      if (existing) {
        await db('products').where('slug', p.slug).update({ title: p.title, short_description: p.short_description, pdf_filename: p.pdf_filename, page_count: p.page_count, dpi: p.dpi, age_group: p.age_group, theme: p.theme, badge: p.badge, is_featured: p.is_featured, is_published: p.is_published, price_eur: p.price_eur, price_usd: p.price_usd });
        updated++;
      } else {
        await db('products').insert({ slug: p.slug, title: p.title, short_description: p.short_description, pdf_filename: p.pdf_filename, page_count: p.page_count, dpi: p.dpi, age_group: p.age_group, theme: p.theme, badge: p.badge, is_featured: p.is_featured, is_published: p.is_published, preview_images: '[]', price_eur: p.price_eur, price_usd: p.price_usd, bonus_filename: null, description: null, cover_image: null });
        inserted++;
      }
    }
    const count = await db('products').count('id as total').first();
...
// UPDATE pdf_filename via POST body
router.post('/update-filenames', async (req, res) => {
  try {
    const { updates } = req.body;
    if (!Array.isArray(updates)) return res.status(400).json({ error: 'updates must be array' });
    let updated = 0;
    for (const { slug, pdf_filename } of updates) {
      const rows = await db('products').where('slug', slug).update({ pdf_filename });
      if (rows > 0) updated++;
    }
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(code)
print('reset.js updated with /update-filenames endpoint')
