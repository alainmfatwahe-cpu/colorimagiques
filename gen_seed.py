...
for fn in nursery:
    base = fn.rsplit('.',1)[0].strip()
    base = base.replace('bb ','Bebe ')
    slug = 'nursery-prints-' + ''.join(c if c.isalnum() or c in '- ' else '' for c in base.lower()).replace(' ','-').replace('+','plus').replace("'",'').replace('sc','scene').replace('Sc','scene')[:50].strip('-')
    title = base
    products.append({'slug': slug, 'title': title, 'theme': 'nursery-prints', 'short_description': f'Nursery Prints - {title}', 'price_eur': 2.99, 'price_usd': 3.49, 'page_count': 1, 'dpi': 300, 'age_group': '0-3', 'badge': 'Populaire' if fn in badge_n else '', 'is_featured': fn in feat_n, 'is_published': True, 'pdf_filename': fn})
